...
for size in sizes:
    ...
    ns.set_bc('west', p=24, dvdn=0)
    ns.set_bc('east', p=0, dvdn=0)
    ...
    def show_info():
        ...
        print "Time: %f" % ns.t
        mid = ns.u.shape[1] / 2
        print "Centre velocitiy (u): %f" % ns.u[-1,mid]
        print "West pressure gradient: %f" % ((ns.p[1,mid] - ns.p[0,mid]) / grid.delta[0])
        print "East pressure gradient: %f" % ((ns.p[-1,mid] - ns.p[-2,mid]) / grid.delta[0])
        print "Error: %f" % (1.0 - ns.u[-1,mid])
        #ns.plot_velocity(1)
