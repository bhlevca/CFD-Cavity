dnl The following is a modified version of acx_mpi.m4,
dnl as obtained from the autoconf macro archive at
dnl http://ac-archive.sourceforge.net/
dnl The original author has been sent patches.
dnl
dnl
...
dnl
dnl @version $Id: acx_mpi.m4,v 1.1.2.3 2003/07/31 09:40:03 wildenhu Exp $
...
[C++], [
	...
	AC_CHECK_PROGS(MPICXX, mpiCC mpCC hcp mpic++, $CXX)
...
if test x = x"$MPILIBS"; then
	...
	AC_LANG_CASE([C], [AC_LINK_IFELSE([#include <mpi.h>
int main(int argc, char **argv)
{
  ...
  return 0;
		...
		AC_MSG_RESULT(yes)],[AC_MSG_RESULT(no)])],
		[C++], [AC_LINK_IFELSE([#include <mpi.h>
int main(int argc, char **argv)
{
  MPI_Init(&argc, &argv);
  return 0;
