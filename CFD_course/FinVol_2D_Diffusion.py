...
class FinVol_2D_Diffusion:
    ...
    def __init__(self, Lx, Ly, Gx, Gy, h, Fi0, FiE, FiW, Nx, Ny, debug = False):
        '''
        Constructor
        '''
    ...
    def calculateSources(self):
        ...
        for j in range(0, self.Ny):
            for i in range(0, self.Nx):
                #west
                if i == 0:
                    self.SuW[j, i] = 2 * self.Gamma[self.W] * self.FiW
                    self.SpW[j, i] = -2 * self.Gamma[self.W]
                else:
                    self.SuW[j, i] = 0
                    self.SpW[j, i] = 0
                ...
                #east
                if i == self.Nx - 1:
                    self.SuE[j, i] = 2 * self.Gamma[self.E] * self.FiE
                    self.SpE[j, i] = -2 * self.Gamma[self.E]
                else:
                    self.SuE[j, i] = 0
                    self.SpE[j, i] = 0
                ...
                if j == self.Ny - 1:
                    self.SuN[j, i] = self.h * self.Fi0 * self.deltaX
                    self.SpN[j, i] = -self.h * self.deltaX
                else:
                    self.SuN[j, i] = 0
                    self.SpN[j, i] = 0
                #south
    ...
    def setBoundaryConditions_WE(self):
        for i in range(0, self.Nx):
            for j in range(0, self.Ny):
                #west
                if i == 0:
                    self.Fi[j, i] = self.FiW

                #east
                if i == self.Nx - 1:
                    self.Fi[j, i] = self.FiE
    ...
    def callTDMA(self):
        ...
        #We know E and W boundary conditions, so lets speed up things
        self.setBoundaryConditions_WE()
