...
#Class definition
class FinVol_QUICK_2D_Conv_Diff:
    '''
    Solve the equation:
    ...
    def calculateQUICKcoefficients(self):
        ...
        for j in range(0, self.Ny):
            for i in range(0, self.Nx):
                ...
                #elif i == self.Nx - 1:
                #    self.aW[j, i] = self.Df[self.W] + 1.0 / 3 * self.Df[self.E] + 6.0 / 8 * self.F[self.W]
                else :
                    self.aW[j, i] = self.Df[self.W] + self.Alpha_w * self.F[self.W]
                #aE
                #if i == 0 :
                #    self.aE[j, i] = self.Df[self.E] + 1.0 / 3 * self.Df[self.W] - 3.0 / 8 * self.F[self.E]
                #elif i == 1:
                #    self.aE[j, i] = self.Df[self.E] - 3.0 / 8 * self.F[self.E]
                if i == self.Nx - 1:
                    self.aE[j, i] = 0
                else:
                ...
                #elif j == self.Ny - 1:
                #    self.aS[j, i] = self.Df[self.S] + 1.0 / 3 * self.Df[self.N] + 6.0 / 8 * self.F[self.S]
                else :
                    self.aS[j, i] = self.Df[self.S] + self.Alpha_s * self.F[self.S]
                #aN
                #if j == 0 :
                    self.aN[j, i] = self.Df[self.N] + 1.0 / 3 * self.Df[self.S] - 3.0 / 8 * self.F[self.N]
                #elif j == 1:
                #    self.aN[j, i] = self.Df[self.N] - 3.0 / 8 * self.F[self.N]
                if j == self.Ny - 1:
                    self.aN[j, i] = 0
                else:
                    self.aN[j, i] = self.Df[self.N] - (1 - self.Alpha_n) * self.F[self.N]

                self.aP[j, i] = self.aW[j, i] + self.aE[j, i] + self.aS[j, i] + self.aN[j, i] + (self.F[self.E] - self.F[self.W]) + (self.F[self.N] - self.F[self.S])
    ...
    def calculateSources(self):
        ...
        for j in range(0, self.Ny):
            for i in range(0, self.Nx):
                ...
                #    SuWp = self.FiW(j, i)
                #else :
                SuWp = 1.0 / 8 * (3 * self.Fi[j, i] - 2 * self.FiW(j, i) - self.FiWW(j, i)) * self.Alpha_w * self.F[self.W]

                SuEp = 1.0 / 8 * (self.FiW(j, i) + 2 * self.Fi[j, i] - 3 * self.FiE(j, i)) * self.Alpha_e * self.F[self.E]

                SuWm = 1.0 / 8 * (3 * self.FiW(j, i) - 2 * self.Fi[j, i] - self.FiE(j, i)) * (1 - self.Alpha_w) * self.F[self.W]

                #if i == self.Nx - 1:
                #    SuEm = self.FiE(j, i)
                #else:
                SuEm = 1.0 / 8 * (2 * self.FiE(j, i) + self.FiEE(j, i) - 3 * self.Fi[j, i]) * (1 - self.Alpha_e) * self.F[self.E]

                #south - north
                #if j == 0:
                #    SuSp = self.FiS(j, i)
                #else:
                SuSp = 1.0 / 8 * (3 * self.Fi[j, i] - 2 * self.FiS(j, i) - self.FiSS(j, i)) * self.Alpha_s * self.F[self.S]

                SuNp = 1.0 / 8 * (self.FiS(j, i) + 2 * self.Fi[j, i] - 3 * self.FiN(j, i)) * self.Alpha_n * self.F[self.N]
                SuSm = 1.0 / 8 * (3 * self.FiS(j, i) - 2 * self.Fi[j, i] - self.FiN(j, i)) * (1 - self.Alpha_s) * self.F[self.S]

                #if j == self.Ny - 1:
                #    SuNm = self.FiN(j, i)
                #else:
                SuNm = 1.0 / 8 * (2 * self.FiN(j, i) + self.FiNN(j, i) - 3 * self.Fi[j, i]) * (1 - self.Alpha_n) * self.F[self.N]

                self.SuDC[j, i] = SuWp + SuEp + SuEm + SuWm + SuSp + SuNp + SuSm + SuNm
            #end j
        #end i
    ...
    def calculateTDMACoefficients(self, i):
        ...
        for j in range(0, self.Ny):
            ...
            if self.Nx == 1:
                ...
                self.C[j] = self.aW[j, i] * self.FiW(j, i) + self.aE[j, i] * self.FiE(j, i) + self.SuDC[j, i]
        #end for j
    ...
    #OLD stuff
    #--------------------------------------------------------------------------------------------
    def calculateStandardQUICKcoefficients(self):
        '''
            2D "a" coeficients for TVD are implementation at page 175 in the Versteeg book 
            
        '''
        self.calculateStandardSources()
        ...
        for j in range(0, self.Ny):
            for i in range(0, self.Nx) :
                ...
                if i == 0:
                    ...
                    self.aW[j, i] = self.Df[self.W] + 6.0 / 8 * self.Alpha_w * self.F[self.W] + 1.0 / 8 * self.Alpha_e * self.F[self.E] + 3.0 / 8 * (1 - self.Alpha_w) * self.F[self.W]

                #aE
                if i == 0 :
                    self.aE[j, i] = self.Df[self.E] + 1.0 / 3 * self.Df[self.E] - 3.0 / 8 * self.F[self.E]
                elif i == 1:
                    self.aE[j, i] = self.Df[self.E] - 3.0 / 8 * self.Alpha_e * self.F[self.E]
                elif i == self.Nx - 1:
                    self.aE[j, i] = 0
                else:
                    self.aE[j, i] = self.Df[self.E] - 3.0 / 8 * self.Alpha_e * self.F[self.E] - 6.0 / 8 * (1 - self.Alpha_e) * self.F[self.E] - 1.0 / 8 * (1 - self.Alpha_w) * self.F[self.W]
                ...
                #aN
                if j == 0 :
                    self.aN[j, i] = self.Df[self.N] + 1.0 / 3 * self.Df[self.N] - 3.0 / 8 * self.F[self.N]
                elif i == 1:
                ...
                if self.Nx == 1:
                    Sp = self.SpS + self.SpN
                    self.aP[j, i] = self.aS[j, i] + self.aN[j, i] + self.aSS[j, i] + self.aNN[j, i] + (self.F[self.N] - self.F[self.S]) - Sp[j, i]
                else :
                    Sp = self.SpE + self.SpW + self.SpS + self.SpN
                    self.aP[j, i] = self.aW[j, i] + self.aE[j, i] + self.aS[j, i] + self.aN[j, i] + self.aSS[j, i] + self.aNN[j, i] + self.aWW[j, i] + self.aEE[j, i]\
                     + (self.F[self.E] - self.F[self.W]) + (self.F[self.N] - self.F[self.S]) - Sp[j, i]

                #TVD deferred correction source term

    #end calculateStandardOUICKcoefficients

    def calculateStandardSources(self):
        '''
        Determine sources on all cardinal points based on boundary conditions
