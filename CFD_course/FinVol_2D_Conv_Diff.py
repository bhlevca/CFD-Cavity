...
class FinVol_2D_Conv_Diff(object):
    ...
    def __init__(self, Lx, Ly, Gx, Gy, FiE, FiW, FiN, FiS, Nx, Ny, u, v, rho, scheme, debug = False):
        ...
        self.EPSILON = 1e-9
        self.maxiter = 5000
        self.urf     = 1.0    #under relaxation factor         
    ...
    # set of fuctions to account for teh cardinal values of the main feature Fi
    def FiW(self, j, i):
        #FiW
        if i == 0:
            FiW = self.Fi0W
    ...
    def calculateTDMACoefficients(self, i):
        ...
        for j in range(0, self.Ny):
            ...
            if self.Nx == 1:
                Su = self.SuS[j, i] + self.SuN[j, i]
                self.C[j] = Su + self.aNN[j, i] * self.FiNN(j, i) + self.aSS[j, i] * self.FiSS(j, i)
            else :
    ...
    def calculateCDSources(self):
        ...
        for j in range(0, self.Ny):
            for i in range(0, self.Nx):
                #west
                if i == 0:
                    self.SuW[j, i] = (2 * self.Df[self.W] + self.F[self.W]) * self.Fi0W
                    self.SpW[j, i] = -(2 * self.Df[self.W] + self.F[self.W])
                else:
                    self.SuW[j, i] = 0
                    self.SpW[j, i] = 0
                ...
                #east
                if i == self.Nx - 1:
                    self.SuE[j, i] = (2 * self.Df[self.E] - self.F[self.E]) * self.Fi0E
                    self.SpE[j, i] = -(2 * self.Df[self.E] - self.F[self.E])
                else:
                    self.SuE[j, i] = 0
                    self.SpE[j, i] = 0
                ...
                #north 
                if j == self.Ny - 1:
                    self.SuN[j, i] = (2 * self.Df[self.N] - self.F[self.N]) * self.Fi0N
                    self.SpN[j, i] = -(2 * self.Df[self.N] - self.F[self.N])
                else:
                    self.SuN[j, i] = 0
                    self.SpN[j, i] = 0
                #south
                if j == 0:
                    self.SuS[j, i] = (2 * self.Df[self.S] + self.F[self.S]) * self.Fi0S
                    self.SpS[j, i] = -(2 * self.Df[self.S] + self.F[self.S])
                else:
                    self.SuS[j, i] = 0
                    self.SpS[j, i] = 0
        #end j
        ...
        for j in range(0, self.Ny):
            for i in range(0, self.Nx) :
                ...
                if i == self.Nx - 1:
                    ...
                    self.aE[j, i] = self.Df[self.E] + max(0, -self.F[self.E])

                #north 
                if j == self.Ny - 1:
                    self.aN[j, i] = 0
                else:
                    self.aN[j, i] = self.Df[self.N] + max(0, -self.F[self.N])
                #south
                if j == 0:
                    self.aS[j, i] = 0
                else:
        ...
        for j in range(0, self.Ny):
            for i in range(0, self.Nx) :
                ...
                if i == 0:
                    ...
                    self.aW[j, i] = self.Df[self.W] + self.F[self.W] / 2

                #east
                if i == self.Nx - 1:
                    self.aE[j, i] = 0
                else:
                    self.aE[j, i] = self.Df[self.E] - self.F[self.E] / 2

                #north 
                if j == self.Ny - 1:
                    self.aN[j, i] = 0
                else:
                    self.aN[j, i] = self.Df[self.N] - self.F[self.N] / 2
                #south
                if j == 0:
                    self.aS[j, i] = 0
                else:
                    self.aS[j, i] = self.Df[self.S] + self.F[self.S] / 2
